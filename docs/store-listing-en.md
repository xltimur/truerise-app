...
LIVE MODE (optional): a live calculation sends the user's birth date, approximate birth time/window, birthplace text and coordinates (when available), and life-event categories/dates/descriptions to a third-party calculation provider over HTTPS, solely to compute the result. It is not linked to any identity (the app has no accounts) and is not used for tracking. The review build may embed a low-budget provider key so live mode can be exercised; Demo mode needs no key. The free live path (proxy / bundled review key) is limited to 3 live requests per rolling 24-hour window; Demo mode is offline and does not count against this quota. A user who enters their own provider API key in Settings calls the provider with that key directly and is not subject to the shared free quota.
...
LIVE MODE (optional) transmits birth date, approximate birth time/window, birthplace text + coordinates (when available), and life-event categories/dates/descriptions to a third-party calculation provider over HTTPS, solely to compute the result; not linked to identity (no accounts), not used for tracking. The Data safety form should match this flow (see docs/play-data-safety.md): encrypted in transit = yes; in-app deletion = yes. The free live path (proxy / bundled review key) is limited to 3 live requests per rolling 24-hour window; Demo mode is offline and does not count against this quota. A user who enters their own provider API key in Settings calls the provider with that key directly and is not subject to the shared free quota.
