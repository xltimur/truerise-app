...
│   └── fonts/                        # Bundled Inter / Source Serif 4 / JetBrains Mono TTFs
├── lib/
...
### 6.7 Fonts (bundled assets)

> **Superseded (2026-06-12):** the original plan below used the
> `google_fonts` package with a bundled-preload pattern. The shipped
> implementation bundles the Inter, Source Serif 4, and JetBrains Mono
> .ttf files directly in `assets/fonts/` (declared in `pubspec.yaml`)
> with no `google_fonts` dependency and no runtime fetching at all.
>
> Original plan (obsolete): `google_fonts` resolves Inter, Source
> Serif 4, and JetBrains Mono; on first run the package downloads and
> caches them; bundle the .ttf files and configure the package to use
> the bundled copies via its preload pattern.
...
- Fonts are bundled local assets (see §6.7) — no font package, no fetch.
...
- Add `pubspec.yaml` dependencies (read-only list — exact versions chosen at pub-add time):
  ...
  - `dio` (`pretty_dio_logger` was planned here but never used; removed 2026-06-12)
...
- `freezed` for models.
- Lucide for icons.
- Bundled font assets for type (no `google_fonts` dependency; removed 2026-06-12).
