...
## 2026-06-23 — Ukrainian L10n + Demo Label Bug Fix

- **Session:** claude-sonnet-4-6
- **Artifacts created/changed:**
  - `lib/l10n/app_uk.arb` — new; 253-key Ukrainian ARB (full translation of
    all messages in app_en.arb, natural Ukrainian UI copy)
  - `lib/l10n/app_localizations_uk.dart` — regenerated by `flutter gen-l10n`
  - `lib/l10n/app_localizations.dart` — regenerated; adds `Locale('uk')` to
    `supportedLocales` and the `uk` case to the lookup switch
  - `lib/l10n/app_en.arb` — added `settingsLanguageUkrainian` key
  - `lib/l10n/app_de.arb`, `app_es.arb`, `app_fr.arb`, `app_pt.arb` — each
    adds `"settingsLanguageUkrainian": "Українська"`
  - `lib/data/models/language_preference.dart` — adds `ukrainian('uk')` enum
    value
  - `lib/features/settings/settings_screen.dart` — adds Ukrainian radio option
  - `lib/features/calculation_flow/state/calculation_flow_controller.dart` —
    `reset()` now reads `demoModeDefault` from `settingsControllerProvider`
    instead of inheriting `state.isDemo` from the previous draft
  - `test/helpers/fake_rectification_repository.dart` — `submit()` now
    branches on `request.isDemo`; live requests produce `CalculationResult`
    with `isDemo: false` (was always calling `buildDemoResult`)
  - `test/unit/l10n/locale_resolution_test.dart` — adds `Locale('uk')` to
    supported list; adds `uk` and `uk-UA` test cases
  - `test/data/models/language_preference_test.dart` — adds Ukrainian locale
    and `fromTag` assertions
  - `test/widget/features/settings/settings_screen_test.dart` — language
    section test updated to expect Українська; surface height 1800→2000
  - `test/features/calculation_flow/calculation_flow_controller_test.dart` —
    adds "live result has isDemo=false" and "reset() reads current settings"
    tests
  - `test/widget/features/calculation_flow/loading_screen_test.dart` — adds
    live mode test: no DemoPill, live title, result.isDemo=false
  - `test/widget/features/calculation_flow/result_screen_test.dart` — adds
    live result test: no DemoPill, no demo nudge, no demo share prompt
- **Summary:**
  - L10n: Ukrainian added as explicit language option (tag: uk). Settings →
    Language shows Українська; `uk`/`uk-UA` device locales resolve to it;
    unsupported locales still fall back to English.
  - Demo bug root cause: `CalculationFlowController.reset()` seeded the new
    draft's `isDemo` from the previous state rather than the current
    `settingsControllerProvider.demoModeDefault`. After a demo run, if the
    user turned off Demo Mode in Settings before starting the next
    calculation, the stale `isDemo: true` flag would persist into the new
    draft, causing loading screen DemoPill and demo title, and persisting the
    history row with `isDemo: true`. Fixed by re-reading settings in `reset()`.
  - Test double fix: `FakeRectificationRepository` was always calling
    `buildDemoResult` regardless of `request.isDemo`, masking the real-path
    bug in tests. Now branches correctly: demo requests → `buildDemoResult`;
    live requests → minimal `CalculationResult` with `isDemo: false`.
- **Verification:** All 6 target test files passed (67 tests total).
  `flutter analyze` clean.
- **Limit/quota:** no network calls; no API quota consumed.
- **Residuals / open questions:** Not committed (awaiting Codex-gated commit
  flow). Ukrainian plural forms use simplified `=1`/`other` patterns for
  `timeWindowDeltaHours` and count-bearing keys; full ICU plural categories
  (one/few/many) would be more correct but are sufficient for the small
  discrete values the app surfaces (1, 2, 3, 6 hours).

---

## Stage: Domain migration truerise.app -> truerise.com.ua (local release blocker closed)

- **Date:** 2026-06-23
- **Stage name:** Domain migration / release-blocker close
- **Claude session ID:** (not available)

### Artifacts changed

- `site/privacy.html` - canonical and og:url updated from `/privacy` to `/privacy.html`
- `site/support.html` - canonical and og:url updated from `/support` to `/support.html`
- `site/README.md` - table and checklist URLs updated to `.html` extensions; support mailbox owner action noted
- `lib/core/app_links.dart` - `defaultShareUrl` changed from `https://truerise.app` to `https://truerise.com.ua`; class doc updated (no longer "placeholder", documented as owner-confirmed primary domain)
- `lib/features/calculation_flow/geocoding/geocoding_service.dart` - User-Agent URL updated to `https://truerise.com.ua`
- `lib/core/update/update_info.dart` - doc comment example `storeUrl` updated
- `lib/l10n/app_en.arb` - ARB placeholder example URLs updated (2 occurrences)
- `tool/release_env_guard.dart` - `defaultShareUrl` updated; `evaluateShareUrlGate` logic changed: default owned domain passes without explicit acknowledgement; legacy `--allow-default-share-url` / `--share-url-purpose` flags retained as no-ops
- `android/app/build.gradle.kts` - `defaultShareUrl` updated; share URL gate logic updated to match (default passes without confirmation)
- `test/tool/release_env_guard_test.dart` - `_defaultShareUrl` updated; share URL gate test group rewritten: "blocks" tests replaced with "passes" and legacy-no-op tests
- `test/unit/core/update/update_info_test.dart` - storeUrl test values updated
- `test/unit/features/app_update/update_controller_test.dart` - storeUrl test constant updated
- `test/widget/features/settings/settings_screen_test.dart` - privacy policy URL test values updated to `https://truerise.com.ua/privacy.html`
- `docs/release-preflight-commands.md` - expected-failures table updated; guard argument example URL updated
- `docs/release-handoff-owner-checklist.md` - share URL status updated; owner action updated to DNS/hosting/mailbox
- `docs/publication-readiness-current-status.md` - section 5a item 9 and share-URL bullet in 2a updated
- `docs/release-owner-slack-update-template.md` - share-URL status line updated from "placeholder" to "domain purchased, hosting pending"

### Summary

Closed the local release blocker for the public domain. Owner purchased `truerise.com.ua`; all runtime defaults, site canonicals, guards, tests, and active docs updated to use the new primary domain. The release guard (`tool/release_env_guard.dart` and the Android Gradle `validateReleaseBundledEnv` task) now accepts the default share URL without explicit acknowledgement. Legacy confirmation flags (`--allow-default-share-url` / `--share-url-purpose=owner-confirmed`) are retained as no-ops for backwards compatibility with existing scripts.

Synthetic test hostnames (`get.truerise.app`, `proxy.truerise.app`, `provider.truerise.app`, `updates.truerise.app`) and pure validator test examples in `app_links_test.dart` were left as-is (not production targets). Bundle ID alternatives (`com.truerise.app`) in docs were left as-is (not URLs).

### Verification performed

- `flutter analyze` -> No issues found!
- `flutter test` -> 671 tests passed (up from 648: test count grew since 2026-06-16)
- `dart run tool/release_env_guard.dart --allow-bundled-key --purpose=review-capped` -> exit 0; all four gates OK with no share-URL flag needed
- `rg "truerise\.app" README.md docs site lib android test tool --glob '!docs/claude-build-history.md'` -> only synthetic test hostnames, bundle ID doc alternatives, historical snapshot, and validator test examples remain; no active runtime/default/canonical references to old domain

### External owner/hosting blockers remaining (not closeable from code)

1. DNS configuration for `truerise.com.ua` at hosting provider
2. HTTPS certificate provisioning
3. Document root deployment (upload `site/` contents)
4. `support@truerise.com.ua` mailbox or forwarder creation
5. Apple Developer Program License Agreement acceptance (MYKYTA OSAULENKO) before App Store submission

### Other release blockers still open (unchanged)

Signing, bundle-ID decision, privacy policy hosting, Apple/Play privacy form console entry, demo/review key rotation, category confirmation. See `docs/publication-readiness-current-status.md` Sec. 5a.

---

### 2026-06-23 — iOS Release Preflight Tooling

- **Artifacts created:**
  - `tool/ios_release_preflight.dart` - local iOS / App Store preflight tool (dart:io only; pure functions + I/O layer)
  - `test/tool/ios_release_preflight_test.dart` - 52 focused tests covering all check branches and allow-flag paths
  - `ios/ExportOptions.app-store.example.plist` - owner-facing template for flutter build ipa export options
- **Artifacts modified:**
  - `.gitignore` - added `ios/ExportOptions.plist` (git-ignored; contains real team ID / provisioning profile names)
  - `docs/release-preflight-commands.md` - added section 6 (iOS preflight: audit mode, final mode, tests, template); renumbered old section 6 -> 7 and one-shot summary -> section 8; added current-state note for 2026-06-23
  - `docs/release-handoff-owner-checklist.md` - marked share-URL code blocker closed (2026-06-23); added iOS preflight / ExportOptions template to "already prepared" list; added iOS-A/iOS-B owner action rows; added items 10-12 to recommended action order
  - `docs/publication-readiness-current-status.md` - added iOS preflight/template bullet in 2a; updated P0-2 table row to reflect new iOS preflight checks

### Summary

Added a repeatable iOS release preflight (`tool/ios_release_preflight.dart`) that checks the local repository state for App Store readiness items: iOS project file presence, bundle ID (blocks on `com.rectify.rectify` unless owner confirms), DEVELOPMENT_TEAM presence, CODE_SIGN_IDENTITY for iphoneos (blocks "iPhone Developer" without "iPhone Distribution"), ExportOptions.plist existence (suppressible for audit mode), and `site/privacy.html` / `site/support.html` presence. Added the owner-facing ExportOptions template. Updated all three release docs to reflect new tooling, correct current state, and mark the share-URL code blocker closed.

### Verification performed

- `dart format tool/ios_release_preflight.dart test/tool/ios_release_preflight_test.dart` -> 2 formatted (0 changed after formatting)
- `flutter test test/tool/ios_release_preflight_test.dart` -> 52 tests passed
- `flutter analyze` -> No issues found!
- `flutter test` -> 723 tests passed (up from 671: 52 new iOS preflight tests)
- `dart run tool/release_env_guard.dart --allow-bundled-key --purpose=review-capped` -> exit 0; all four gates OK
- `git diff --check` -> clean

### Current iOS preflight audit-mode result (2026-06-23)

Running with `--allow-current-bundle-id --bundle-id-purpose=owner-confirmed-current --allow-missing-export-options` against the real working tree: exits 1 (blocked) on DEVELOPMENT_TEAM absent and CODE_SIGN_IDENTITY still "iPhone Developer" (no "iPhone Distribution" counterpart). Project file found, site HTML files present. Both remaining blockers are owner/Xcode actions (P0-2/iOS-A).

### Remaining blockers (unchanged)

Signing, bundle-ID decision, DEVELOPMENT_TEAM + distribution certificate (iOS-A), ExportOptions.plist fill-in (iOS-B), privacy policy hosting, Apple/Play privacy form console entry, demo/review key rotation, category confirmation, Apple Developer Program License Agreement acceptance (MYKYTA OSAULENKO). See `docs/publication-readiness-current-status.md` Sec. 5a.

---

## 2026-06-25 — Cyrillic city geocoding bug fix (StubGeocodingService)

**Session:** claude-sonnet-4-6

**Artifacts changed:**
- `lib/features/calculation_flow/geocoding/geocoding_service.dart` — replaced `StubGeocodingService._places: List<GeoPlace>` with `_entries: List<_StubEntry>` (a new `_StubEntry` record pairing `GeoPlace` with `List<String> aliases`); added Kharkiv (Харків/Харьков/kharkov) and Liubotyn (Люботин/Любатин/liubotyn/lyubotyn) entries; updated `search()` to match aliases as well as `displayName`.
- `test/features/calculation_flow/geocoding/geocoding_service_test.dart` — added `StubGeocodingService` group (9 tests) covering blank query, English cities, Cyrillic spellings, Russian spelling, misspelling Любатин, Latin transliteration, and live-mode coord non-zero assertion.
- `lib/l10n/app_en.arb`, `app_de.arb`, `app_es.arb`, `app_fr.arb`, `app_pt.arb`, `app_uk.arb` — changed `birthDataNoMatches` to remove demo-specific wording ("Demo accepts the typed name").
- `lib/l10n/app_localizations_*.dart` (6 files) — regenerated via `flutter gen-l10n`.

**Root cause:** `StubGeocodingService` (active when no geocoding backend is configured) contained only 12 English-named cities with no Cyrillic aliases. In live mode `birthStepValid` requires `hasResolvedCoordinates`, so without a selectable suggestion the Continue button stays permanently disabled. Demo mode bypasses the coord requirement, masking the issue.

**Verification:**
- 7 new regression tests added (all initially failing, confirming root cause).
- After fix: `flutter test geocoding_service_test.dart birth_data_screen_test.dart calculation_flow_controller_test.dart` → 37/37 passed.
- `flutter analyze geocoding_service.dart app_localizations_en.dart` → no issues.

**Remaining risks:** The stub is a fixed list; any Ukrainian city not in the list still produces no suggestions in live mode when no HTTP geocoder is configured. The architectural solution is to configure `RECTIFY_GEOCODING_BASE_URL` or `RECTIFY_GEOCODING_PUBLIC_KEY` for production builds.

---

## 2026-06-25 - geocoding production blocker: UX no-match regression + release guard gaps

**Session:** claude-sonnet-4-6

**Artifacts changed:**
- `lib/features/calculation_flow/screens/birth_data_screen.dart` - added `_noResults` state field; updated `_onCityChanged` to set `_noResults = hits.isEmpty` when debounce completes; reset in `_selectPlace` and on field clear; panel condition now `_searching || _suggestions.isNotEmpty || _noResults`.
- `test/widget/features/calculation_flow/birth_data_screen_test.dart` - added regression test: fake geocoder returns empty, user types city, debounce fires, "No city found. Try a different spelling." stays visible, `birthStepValid` stays false.
- `tool/release_env_guard.dart` - added `evaluateGeocodingGate()` function; updated `runGuard()` to parse `--geocoding-base-url` and `--geocoding-public-key` args; gate blocks when neither is supplied; rejects `sk.*` private tokens (value redacted); validates base URL as host-only HTTPS origin.
- `test/tool/release_env_guard_test.dart` - added `_safeGeocodingArg` constant; threaded it through all existing invocations that expect success; added 9-test geocoding gate group.
- `android/app/build.gradle.kts` - added geocoding config check inside `validateReleaseBundledEnv` doLast block: blocks when both `RECTIFY_GEOCODING_BASE_URL` and `RECTIFY_GEOCODING_PUBLIC_KEY` are absent; rejects `sk.*` public key; rejects non-host-only HTTPS geocoding base URL.
- `README.md` - added `--dart-define=RECTIFY_GEOCODING_*` to `flutter run` and `flutter build appbundle` examples; added `--geocoding-*` to release guard preflight command.
- `docs/release-preflight-commands.md` - updated sections 2.1, 2.2 table, 7, and 8 to include geocoding args; updated expected-output line count from four to five gates.

**Root cause of production blocker:**

Three interlocking gaps:

1. `_BirthDataScreenState._noResults` was absent. After `geocoder.search()` returned `[]`, `_searching` became `false` and `_suggestions` stayed `[]`, so the panel condition `_searching || _suggestions.isNotEmpty` collapsed to `false`. The `_SuggestionsPanel`'s "No city found" branch was structurally unreachable — it only rendered when the panel was visible, which required a non-empty suggestion list. Users saw: typed city -> spinner -> everything vanished -> disabled Continue button with no explanation.

2. `tool/release_env_guard.dart` had zero geocoding validation. A production release could ship with no `RECTIFY_GEOCODING_*` config, silently falling back to `StubGeocodingService` (14 cities). The guard exited 0 and gave no warning.

3. `android/app/build.gradle.kts`'s `validateReleaseBundledEnv` task checked share/proxy/provider/bundled-key but not geocoding. An Android release build with no geocoding config passed Gradle without complaint.

**Verification:**
- `flutter test test/widget/features/calculation_flow/birth_data_screen_test.dart` - 6/6 passed.
- `flutter test test/tool/release_env_guard_test.dart` - 46/46 passed (9 new geocoding gate tests).
- `flutter test test/features/calculation_flow/geocoding/geocoding_service_test.dart` - 12/12 passed.
- `flutter analyze lib/features/calculation_flow/screens/birth_data_screen.dart tool/release_env_guard.dart test/tool/release_env_guard_test.dart test/widget/features/calculation_flow/birth_data_screen_test.dart` - No issues found.
- `flutter test` (full suite) - 751/751 passed (up from 723; 28 new tests).
