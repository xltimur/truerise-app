...
| Hosted Privacy Policy URL | 🟡 Partial — app wiring done (2026-06-12), owner/legal URL pending | Phase 7 added an in-app `PrivacyPolicyScreen`. The `url_launcher` wiring has since landed, config-gated: a valid bare-HTTPS `--dart-define=TRUERISE_PRIVACY_POLICY_URL` opens from Settings via `LaunchMode.inAppBrowserView`; empty (default), unsafe, or failed launches fall back to the in-app screen. | Legal team to publish the canonical privacy policy at a stable public URL; build with that URL in `TRUERISE_PRIVACY_POLICY_URL` and use the same URL in the store consoles/listing. |
| Final bundle ID + app name | 🟡 Partial — display name done (2026-06), bundle ID owner-pending | Display name is now **TrueRise** (iOS `CFBundleDisplayName`, Android `android:label`; was "Rectify" at the original audit). Bundle ID is still `com.rectify.rectify`; the real bundle ID requires legal clearance per PRD §2 and §15.3. | Product to finalize bundle ID + confirm App Store name (legal/trademark) before submission; bundle-ID change is one line in `ios/Runner.xcodeproj` + `android/app/build.gradle.kts`. |
| App icon glyph (clay clock-quadrant) | ✅ Done in-repo (2026-06) | Real icon assets have landed: iOS `AppIcon.appiconset` (incl. 1024×1024) + Android adaptive icon (`mipmap-*` foreground/background + legacy). At the original audit (2026-05-20) the default Flutter glyph was still in place. | Store-console icon review (and screenshots from a build carrying the icon) remains owner scope at submission. |
