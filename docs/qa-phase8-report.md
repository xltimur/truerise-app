...
| Final bundle ID + app name | 🟡 Partial — display name + Bundle ID done locally (2026-06), owner store/account actions pending | Display name is now **TrueRise** (iOS `CFBundleDisplayName`, Android `android:label`; was "Rectify" at the original audit). Bundle/package ID is now `ua.com.truerise.app` for iOS + Android. | Product to confirm App Store name/legal/trademark; owner creates store records, signing, and provisioning with `ua.com.truerise.app`. |
