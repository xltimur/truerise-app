# Store Screenshots - Brazilian Portuguese Tier 1 (TrueRise)

**Impl Run D.4 + current-plan refresh · status updated 2026-07-02.**

Current five-frame store screenshot pack for `pt-BR`. Raw PNGs are captured from the shipped Flutter UI in offline Demo mode; final caption-overlay PNGs are generated under `composited/` from the same raw frames and the reviewed caption plan in `docs/store-listing-tier1-localized.md §3`.
...
| # | File | Route | Caption |
|---|---|---|---|
| 1 | `01-problem-hook.png` | `/calc/window` | Não sabe seu horário de nascimento exato? |
| 2 | `02-life-events.png` | `/calc/events` | Adicione os eventos de vida que você lembra. |
| 3 | `01-result-hero.png` | `/calc/result/:id` | Estime seu horário de nascimento e seu ascendente. |
| 4 | `02-evidence-breakdown.png` | `/calc/result/:id/evidence` | Veja os indícios por trás de cada horário candidato. |
| 5 | `03-privacy-demo-settings.png` | `/settings` | Privado por padrão. Experimente grátis, off-line. |
...
The share button label is localized as `Compartilhar resultado`. The emitted text is built by `lib/core/sharing/share_copy_builder.dart`, uses `AppLocalizations`, and remains privacy-safe: estimated time, rising sign, confidence, brand, and public share URL only. No birth date, birthplace, life events, labels, API IDs, or other PII are included.

```
Meu resultado de horário de nascimento com TrueRise:
7:14 AM · Ascendente em Gemini · Nível de confiança: 78%

Calculado com TrueRise: retificação do horário de nascimento
Descubra seu horário de nascimento: https://truerise.com.ua
```
