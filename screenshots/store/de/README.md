...
Current five-frame store screenshot pack for `de`. Raw PNGs are captured from the shipped Flutter UI in offline Demo mode; final caption-overlay PNGs are generated under `composited/` from the same raw frames and the reviewed caption plan in `docs/store-listing-tier1-localized.md §3`.
...
| # | File | Route | Caption |
|---|---|---|---|
| 1 | `01-problem-hook.png` | `/calc/window` | Du kennst deine genaue Geburtszeit nicht? |
| 2 | `02-life-events.png` | `/calc/events` | Füge die Lebensereignisse hinzu, an die du dich erinnerst. |
| 3 | `01-result-hero.png` | `/calc/result/:id` | Schätze deine Geburtszeit und deinen Aszendenten. |
| 4 | `02-evidence-breakdown.png` | `/calc/result/:id/evidence` | Sieh die Hinweise hinter jeder Kandidatenzeit. |
| 5 | `03-privacy-demo-settings.png` | `/settings` | Privat von Anfang an. Teste kostenlos, offline. |
...
The share button label is localized as `Ergebnis teilen`. The emitted text is built by `lib/core/sharing/share_copy_builder.dart`, uses `AppLocalizations`, and remains privacy-safe: estimated time, rising sign, confidence, brand, and public share URL only. No birth date, birthplace, life events, labels, API IDs, or other PII are included.

```
Mein TrueRise-Ergebnis zur Geburtszeit:
7:14 AM · Aszendent Gemini · 78 % Wahrscheinlichkeit

Berechnet mit TrueRise: Geburtszeit-Korrektur
Finde deine Geburtszeit: https://truerise.com.ua
```
